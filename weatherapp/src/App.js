...
const apiKey = 'L6SswBkkA0CEZ96VQxgjjeGXptnjW9gN';
...
{
  var link;
  link = 'http://dataservice.accuweather.com/locations/v1/cities/search?apikey=' + apiKey +'&q='+ searchTerm+'&details=true';
  return link;
}
...
function jointWeatherLink(cityKey)
{
  ...
  let url = 'http://dataservice.accuweather.com/forecasts/v1/daily/5day/' + cityKey + '?apikey=' + apiKey + '&details=true'; 
...
{
  let stringhelper = string;
 return stringhelper.slice(10);
...
try{
  ...
  _handleChange(event) {
    this._setIndex(event);
    this._setValues();
     console.log("Index: ");
    console.log(event.target.value);
    console.log(this.state.dayIdx);
  }
  ...
  render() {
    let iconUrl =  "https://developer.accuweather.com/sites/default/files/01-s.png";
    ...
    return (
      ...
      <CityButton value="Havana" className = "chooseCityButton" event = {()=>this._findCity('London')}/>
      <CityButton value="London" className = "chooseCityButton" event = {()=>this._findCity('Havana')}/>
      <input type="range" className="daySlider" min="0" max="4" step="1" value = {this.state.dayIdx} onChange={this._setIndex} ></input>
       <Timer offset = {this.state.timeOffset} />
...
{
  console.log(props);
  return (<div style ={{gridRow: "span 2", background:"black", color:"white", padding: "1rem", borderRadius: "10px"}}> <div style = {{gridColumn: "span 1"}}>{props.printProps.maxTempStr}
